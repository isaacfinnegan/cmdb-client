...
$(NAME)-$(VERSION): $(FILES) $(EXES) $(CONFS) $(LIBS)
	...
	if [ -n "$(LIBS)" ]; then $(ENSURE_DIR) $(NAME)-$(VERSION)/lib/perl/$(PERL_VERSION_INC) && $(MAKE_LIBS) $(LIBS) $(NAME)-$(VERSION)/lib; tar cf - $(LIBS) | (cd $(NAME)-$(VERSION)/lib/perl/$(PERL_VERSION_INC) && tar xf -);fi
