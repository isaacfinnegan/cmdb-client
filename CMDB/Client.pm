...
$key
Optional parameter to specify the entity key used in the REST PUT.   NOTE: if this parameter is excluded, 
a POST may be used if the entity key is not included in the \%record
...
sub saveRec
{
    ...
    $config = $config || {};
    return 'error: user and pass required'
      unless ( $config->{'user'} && $config->{'pass'} );
    my $hostname    = $config->{'host'} || $host;
    my $http_method = $config->{'http'} || $http;
    my $port = $http_method eq 'http' ? '80' : '443';
    my $path  = $config->{'path'}  || $api_path;
    ...
    if ($rec_key)
    {
        print "looking up via : $url\n" if $DEBUG;
        $response = $ua->get("$url$rec_key");
        print STDERR "got " . $response->code . " for $url$rec_key\n" if $DEBUG;
    }
    if ( $rec_key && $response->code == 200 )
    {
        print STDERR "found in cmdb, doing PUT\n" if $DEBUG;
